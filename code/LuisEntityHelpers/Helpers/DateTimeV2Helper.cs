// <copyright file="DateTimeV2Helper.cs" company="Matt Lacey">
...
{
    public class DateTimeV2Helper : HelperCore
    {
        public DateTimeV2Helper()
        {
            this.Settings = DateTimeHelperSettings.Default;
        }

        public DateTimeV2Helper(DateTimeHelperSettings settings)
        {
        ...
        {
            var helper = new DateTimeV2Helper(settings);

            return helper.Parse(entityRecommendation);
        }

        public static IParseResponse ParseEntity(LuisEntity entityRecommendation, DateTimeHelperSettings settings = null)
        {
            var helper = new DateTimeV2Helper(settings);
        ...
        {
            throw new NotImplementedException("Not Supported for DateTimeV2. Use `Parse(LuisEntity)` instead.");
        }

        public new IParseResponse Parse(string luisEntity)
        {
            if (luisEntity == null)
            {
                throw new ArgumentNullException(nameof(luisEntity));
            }

            LuisEntity resp;
            try
            {
                resp = JsonConvert.DeserializeObject<LuisEntity>(
                    luisEntity,
                ...
                {
                    throw new ArgumentException(nameof(luisEntity));
                }
            }
            catch (Exception e)
            {
                throw new ArgumentException(nameof(luisEntity), e);
            }

            return this.Parse(resp);
        }

        public IParseResponse Parse(LuisEntity entity)
        {
            if (entity == null)
            {
            ...
            DateTimeV2ParseResponse result;

            if (entity.Type?.StartsWith(Builtin.DateTimeV2.Prefix) ?? false)
            {
                ...
                switch (entity.Type)
                {
                    case Builtin.DateTimeV2.Date:
                        if (entity.Resolution.FirstValue.Type != "date")
                        {
                            throw new ArgumentException("Resolution was not of the expected type 'date'");
                        }

                        var resolvedDate = entity.Resolution.FirstValue.Timex;

                        this.ParseDate(entity, resolvedDate, ref resType, ref datetime, ref starttime, ref endtime);
                    ...
                    case Builtin.DateTimeV2.Time:
                        if (entity.Resolution.FirstValue.Type != "time")
                        {
                            throw new ArgumentException("Resolution was missing expected 'time' key");
                        }

                        var resolvedTime = entity.Resolution.FirstValue.Timex;
                    ...
                    case Builtin.DateTimeV2.Duration:
                        ...
                        if (entity.Resolution.FirstValue.Type != "duration")
                        {
                            throw new ArgumentException("Resolution was missing expected 'duration' key");
                        }

                        var resolvedDuration = entity.Resolution.FirstValue.Timex;
                ...
                result = new DateTimeV2ParseResponse(
                                                   entity: entity,
        ...
        private void ParseDate(LuisEntity entity, string resolvedDate, ref DateTimeResolutionType resType, ref DateTime? datetime, ref DateTime? starttime, ref DateTime? endtime)
        {
